...
from unimate.models.diffusion.losses import normal_kl, discretized_gaussian_log_likelihood
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.l2_loss = lambda a, b: (a - b) ** 2  # element-wise; masks are applied afterwards

    def temporal_spatial_masked_l2(self, target, pred, temp_mask, spat_mask, lengths, n_joints):
        """Compute masked L2 loss over temporal and spatial dimensions.
        Args:
            target, pred: (bs, n_joints, feat_dim, n_frames)
            temp_mask: (bs, 1, 1, n_frames)
            spat_mask: (bs, 1, 1, n_joints)
            lengths, n_joints: (bs,) number of valid frames, number of valid joints
        Returns:
            mse_loss_val: (bs,) MSE loss values for each sample in the batch
        """
    ...
    def geodesic_loss(self, target, pred, temp_mask, spat_mask, lengths, n_joints):
        """Compute masked geodesic loss over temporal and spatial dimensions.
        Args:
            target, pred: (bs, n_joints, feat_dim, n_frames)
            temp_mask: (bs, 1, 1, n_frames)
            spat_mask: (bs, 1, 1, n_joints)
            lengths, n_joints: (bs,) number of valid frames, number of valid joints
        Returns:
            loss_val: (bs,) geodesic loss values for each sample in the batch
        """
        rots_target = rotation_6d_to_matrix_safe(target.permute(0, 3, 1, 2)[..., 3:9]) # (bs, F, J, 3, 3)
        rots_pred = rotation_6d_to_matrix_safe(pred.permute(0, 3, 1, 2)[..., 3:9]) # (bs, F, J, 3, 3)

        loss = geodesic_distance(rots_pred, rots_target).permute(0, 2, 3, 1) # (bs, J, 1, F)
        temp_masked_loss = loss * temp_mask.float()
        spat_temp_masked_loss = (temp_masked_loss * spat_mask.float().transpose(1,3))
        loss = sum_flat(spat_temp_masked_loss)  # gives \sigma_euclidean over unmasked elements
        non_zero_elements = lengths * n_joints  # (bs,) number of non-masked joints * frames
        loss_val = loss / non_zero_elements # (bs,)
        return loss_val
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        :param model: the model to compute losses for.
        :param x_start: the [N x J x D x F] tensor of original inputs.
        :param t: a batch of timestep indices.
        :param model_kwargs: extra model keyword arguments; ``model_kwargs["cond"]``
            holds the conditioning dict (masks, lengths, normalization stats).
        :param noise: the noise that was added to x_start to produce x_t.
        ...
        temp_mask = model_kwargs['cond']['lengths_mask'] # (bs, 1, 1, F)
        gt_n_frames = model_kwargs['cond']['motion_length'] # (bs,)
        joint_mask = model_kwargs['cond']['joint_mask'] # (bs, 1, 1, J)
        gt_n_joints = model_kwargs['cond']['n_joints'] # (bs,)

        if noise is None:
            noise = th.randn_like(x_start) # (bs, J, D, F)
        x_t = self.q_sample(x_start, t, noise=noise) # (bs, J, D, F)
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            terms["l_simple"] = self.temporal_spatial_masked_l2(
                ...
                pred=model_output,
                temp_mask=temp_mask,
                spat_mask=joint_mask,
                lengths=gt_n_frames,
                n_joints=gt_n_joints
            )
            terms["loss"] = terms["l_simple"].clone()

            # Geodesic loss requires denormalized predictions
            if self.lambda_geo > 0.:
                mean = model_kwargs['cond']['mean'][..., None]  # (bs, J, D, 1)
                std = model_kwargs['cond']['std'][..., None]  # (bs, J, D, 1)
                target_denorm = (target * std) + mean
                output_denorm = (model_output * std) + mean
