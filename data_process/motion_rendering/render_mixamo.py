"""Render multi-view frames for pre-animated Mixamo characters (EEVEE).

Consumes animated GLB/FBX files produced by
``mesh_animation/animate_mixamo.py`` (mesh + armature + baked animation)
and renders each as ``{file_stem}/v00{0..3}/*.png`` — the layout consumed
by ``vlm_caption/caption_motion.py``.

Character meshes are kept intact (body / hair / clothes are often separate
meshes, so no main-mesh filtering is applied).

Usage (plain python — EEVEE needs the pip ``bpy`` module's GPU context):
    python -m data_process.motion_rendering.render_mixamo \
        --data_dir outputs/mixamo_characters --output_dir <render_dir>
"""
...
def render_animated_file(path, output_dir, args):
    """Render multi-view frames for one pre-animated character file."""
    ...
    # The animated file carries exactly one baked action: the one bound to
    # the armature. Falling back to a different action would render a motion
    # the matching NPZ never contains.
...
def main():
    parser = argparse.ArgumentParser(
        description="Render multi-view frames for animated Mixamo characters (EEVEE).")
    parser.add_argument("--data_dir", type=str, required=True,
                        help="Directory of animated GLB/FBX files from "
                             "mesh_animation/animate_mixamo.py.")
    ...
    logger.info(f"Found {len(files)} animated files in {data_dir}")
    ...
    for path in files:
        try:
            render_animated_file(path, args.output_dir, args)
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.exception("render_mixamo failed")
        sys.exit(1)
